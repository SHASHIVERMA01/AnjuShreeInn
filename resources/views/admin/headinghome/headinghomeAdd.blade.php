    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="p-4 border rounded" style="background: #fff">
                <form class="row g-3" method="post" action="{{route('homeheading.store', $headinghome->id ?? '')}}" enctype="multipart/form-data">
                    {{csrf_field()}}
                    <input class=" form-control" id="cimage" name="id" value="{{ $headinghome->id ?? '' }}" type="text" hidden>	

                    <div class="col-md-12">
                    <h4>Heading</h4>
                        <label for="validationDefault02" class="form-label"></label>
                        <input class=" form-control" id="cimage" name="heading" type="text" value="{{ $headinghome->heading ?? '' }}"
                            {{(request()->segment(5)) ? '' : 'required'}}>
                        <p style="color:red">{{$errors->first('heading')}}</p>
                    </div>

                    <div class="col-md-12">
                    <h4>Heading Two</h4>
                        <label for="validationDefault02" class="form-label"></label>
                        <input class=" form-control" id="cimage" name="heading_one" type="text" value="{{ $headinghome->heading_one ?? '' }}"
                            {{(request()->segment(5)) ? '' : 'required'}}>
                    ...
                    <h4>Content</h4>
                        <label for="validationDefault02" class="form-label"></label>
                        <textarea name="content" id="editor1" rows="5" cols="80"  class="form-control">{{ $headinghome->content ?? '' }}</textarea>
